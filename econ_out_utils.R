# ============================================================================
# ECONOMIC EFFICIENCY UTILITIES — econ_out_utils.R
# ============================================================================
# Pure helpers + one orchestrator for the Economic Outcomes section:
# market sizing, supplier entry dynamics, buyer–supplier networks,
# relative prices, and single-bid competition diagnostics.
# Sourced SECOND by global.R (after utils_shared.R).
#
...
# TABLE OF CONTENTS
# -----------------
#   [EC-01]  SETUP — packages + plotly 'text' aesthetic registration
#   [EC-02]  save_plot() — PNG writer used by the pipeline in save mode
#   [EC-03]  CPV CLUSTER HELPERS (make_cpv_cluster_legend, add_cpv_cluster)
#   [EC-04]  FLAGS & BINS (add_single_bid_flag, add_price_bins_usd, wrap_strip)
#   [EC-05]  CPV LOOKUP (build_cpv_lookup, attach_cpv_labels)
#   [EC-06]  YEAR AXIS RULES + NETWORK YEAR LIMITS
#   [EC-07]  MARKET SIZING (summarise_market_size + counts/value/bubble plots)
#   [EC-08]  SUPPLIER ENTRY (per-market heatmaps + aggregate fallback)
#   [EC-09]  BUYER–SUPPLIER NETWORKS (plot_buyer_supplier_networks)
#   [EC-10]  RELATIVE PRICES (add_relative_price + density/year/top-N plots)
#   [EC-11]  SINGLE-BID / COMPETITION (lollipop plot suite)
#   [EC-12]  MAIN PIPELINE: run_economic_efficiency_pipeline()
#
# Entry point: run_economic_efficiency_pipeline() [EC-12] returns a named list
# consumed by the server as econ$analysis (see DEVELOPER_GUIDE.md §4.2).
# ============================================================================
...
add_price_bins_usd <- function(df, value_col = NULL) {
  # Auto-detect the best available price column when not specified.
  # Priority: bid_priceusd → bid_price → lot_estimatedpriceusd → lot_estimatedprice
...
# ------------------------------------------------------------------------
# 5a) Aggregate supplier entry — for datasets without CPV market categories
# These two functions mirror compute_supplier_entry / plot_supplier_shares_heatmap
# but operate across the whole dataset (no cpv_cluster grouping), producing a
# single stacked area chart of new vs repeat suppliers over time.
# ------------------------------------------------------------------------

compute_supplier_entry_aggregate <- function(df,
                                             supplier_id_col = "bidder_masterid",
                                             year_col = "tender_year") {
  # Validate required columns
  missing <- setdiff(c(supplier_id_col, year_col), names(df))
  if (length(missing) > 0)
  ...
  df2 <- df %>%
    dplyr::filter(
      ...
      nchar(as.character(.data[[supplier_id_col]])) > 0,
      !is.na(.data[[year_col]])
    ) %>%
    # First year each supplier was ever seen (globally, not per market)
    dplyr::group_by(.data[[supplier_id_col]]) %>%
    dplyr::mutate(first_year = min(.data[[year_col]], na.rm = TRUE)) %>%
    dplyr::ungroup() %>%
    dplyr::mutate(is_new = .data[[year_col]] == first_year)
  ...
  df2 %>%
    ...
    dplyr::rename(tender_year = 1)
}

plot_supplier_entry_aggregate <- function(agg_stats,
                                          supplier_id_col = "bidder_masterid") {
  if (is.null(agg_stats) || nrow(agg_stats) == 0) return(NULL)
  
  # Human-readable label for whichever ID tier was used
  id_label <- dplyr::case_when(
    supplier_id_col == "bidder_masterid" ~ "Master ID",
    supplier_id_col == "bidder_id"       ~ "Supplier ID",
    TRUE                                 ~ "Supplier Name"
  )
  
  plot_long <- agg_stats %>%
    tidyr::pivot_longer(
      cols      = c(n_new_suppliers, n_repeat_suppliers),
      names_to  = "type",
      values_to = "n"
    ) %>%
    dplyr::mutate(
      type = dplyr::recode(type,
                           n_new_suppliers    = "New Suppliers",
                           n_repeat_suppliers = "Repeat Suppliers"
      ),
      hover = paste0(
        "<b>", type, "</b><br>",
        "Year: ", tender_year, "<br>",
        "Count: ", scales::comma(n), "<br>",
        "Total suppliers that year: ", scales::comma(n_suppliers)
      )
    )
  
  ggplot2::ggplot(
    plot_long,
    ggplot2::aes(x = factor(tender_year), y = n,
                 fill = type, text = hover)
  ) +
    ggplot2::geom_col(position = "stack", width = 0.72, alpha = 0.88) +
    ggplot2::scale_fill_manual(
      values = c("New Suppliers"    = "#e74c3c",
                 "Repeat Suppliers" = "#3498db"),
      name   = NULL
    ) +
    ggplot2::scale_y_continuous(labels = scales::comma) +
    ggplot2::labs(
      x       = "Year",
      y       = paste0("Suppliers (identified by ", id_label, ")"),
      caption = paste0("Supplier identity column: '", supplier_id_col, "'")
    ) +
    ggplot2::theme_minimal(base_size = 12) +
    ggplot2::theme(
      axis.text.x      = ggplot2::element_text(angle = 45, hjust = 1),
...
plot_relative_price_by_year <- function(df) {
  ...
  ggplot2::ggplot(yr_stats, ggplot2::aes(x = tender_year)) +
    ...
    ggplot2::scale_x_continuous(breaks = function(x) seq(ceiling(x[1]), floor(x[2]), by = 1)) +  # integer years only
...
top_buyers_by_relative_price <- function(df,
  ...
  df %>%
    # Contracts with no buyer identifier cannot be attributed to anyone —
    # without this filter they aggregate into a meaningless "NA" buyer bar.
    dplyr::filter(!is.na(.data[[buyer_col]]),
                  trimws(as.character(.data[[buyer_col]])) != "",
                  toupper(trimws(as.character(.data[[buyer_col]]))) != "NA") %>%
...
plot_top_buyers_relative_price <- function(top_buyers_df, label_max_chars = 30,
  ...
  # Detect currency label from the data if not supplied
  if (is.null(currency_label)) {
    currency_label <- if ("bid_pricecurrency" %in% names(top_buyers_df)) {
      vals <- top_buyers_df[["bid_pricecurrency"]]
      vals <- vals[!is.na(vals) & nchar(as.character(vals)) == 3]
      if (length(vals) > 0) toupper(names(sort(table(vals), decreasing=TRUE))[1]) else "NC"
    } else "NC"
  }
  cur_prefix <- if (currency_label == "USD") "$" else paste0(currency_label, " ")
  ...
  dfp <- top_buyers_df %>%
    ...
    dplyr::mutate(
      ...
      hover_text = paste0(
        ...
        if (has_value)
          paste0("<br>Total contract value: ", cur_prefix,
...
# retained for pipeline call-compatibility
...
plot_top_buyers_single_bid <- function(df,
                                       ...
                                       min_tenders     = 30,
                                       label_max_chars = 35) {
  # Auto-detect the buyer ID column when not specified.
  # Priority: buyer_masterid → buyer_id → buyer_name
  if (is.null(buyer_id_col)) {
    buyer_id_col <- intersect(c("buyer_masterid", "buyer_id", "buyer_name"), names(df))[1]
    if (length(buyer_id_col) == 0 || is.na(buyer_id_col))
      return(ggplot2::ggplot() + ggplot2::labs(subtitle = "No buyer identifier column found"))
  }
  if (!buyer_id_col %in% names(df))
    return(ggplot2::ggplot() + ggplot2::labs(subtitle = paste("Column not found:", buyer_id_col)))
...
run_economic_efficiency_pipeline <- function(df,
  ...
  # Normalise column structure for limited / national datasets before any
  # analysis.  This creates canonical alias columns (buyer_masterid,
  # buyer_buyertype, bidder_masterid, lot_productcode, bid_priceusd) from
  # whatever alternative names are present in the uploaded file.
  ...
  } else if ("buyer_id" %in% names(df)) {
    dplyr::n_distinct(df$buyer_id, na.rm = TRUE)
  } else if ("buyer_name" %in% names(df)) {
    dplyr::n_distinct(df$buyer_name, na.rm = TRUE)
  } else NA_integer_
  
  n_unique_bidders <- if ("bidder_masterid" %in% names(df)) {
    dplyr::n_distinct(df$bidder_masterid, na.rm = TRUE)
  } else if ("bidder_id" %in% names(df)) {
    dplyr::n_distinct(df$bidder_id, na.rm = TRUE)
  } else if ("supplier_name" %in% names(df)) {
    dplyr::n_distinct(df$supplier_name, na.rm = TRUE)
  } else if ("bidder_name" %in% names(df)) {
    dplyr::n_distinct(df$bidder_name, na.rm = TRUE)
  } else NA_integer_
  ...
  # ----------------------------------------------------------------------
  # C) Supplier entry
  # Three-tier supplier ID priority: bidder_masterid → bidder_id → bidder_name
  # When CPV market data is available: compute per-market heatmaps.
  # When CPV data is absent: fall back to a single aggregate stacked bar chart.
  # ----------------------------------------------------------------------
  supplier_stats     <- NULL
  suppliers_entrance <- NULL
  unique_supp        <- NULL
  supplier_entry_agg <- NULL
  ...
  if (!is.na(sup_id_col) && "tender_year" %in% names(df)) {
    has_cpv <- "cpv_cluster" %in% names(df) && any(!is.na(df$cpv_cluster))
    
    if (has_cpv) {
      # --- Per-market path (CPV present) ---
      ...
      supplier_stats <- tryCatch(
        compute_supplier_entry(df_supplier_analysis, supplier_id_col = sup_id_col),
        error = function(e) NULL
      )
      if (!is.null(supplier_stats)) {
        suppliers_entrance <- tryCatch(
          plot_supplier_shares_heatmap(supplier_stats),
          error = function(e) NULL
        )
        unique_supp <- tryCatch(
          plot_unique_suppliers_heatmap(df_supplier_analysis, supplier_id_col = sup_id_col),
          error = function(e) NULL
        )
        # Sanity check: verify cluster sets match
      ...
      agg_stats <- tryCatch(
        compute_supplier_entry_aggregate(df_supplier_analysis, supplier_id_col = sup_id_col),
        error = function(e) NULL
      )
      if (!is.null(agg_stats)) {
        supplier_entry_agg <- tryCatch(
          plot_supplier_entry_aggregate(agg_stats, supplier_id_col = sup_id_col),
          error = function(e) NULL
        )
      }
    }
  } else {
    message("Supplier entry analysis skipped: no suitable supplier ID column found ",
            "(checked bidder_masterid, bidder_id, bidder_name) or tender_year missing.")
  }
  ...
  if ("single_bid" %in% names(df) && any(!is.na(df$single_bid))) {
    ...
    # 6) top buyers — auto-detect the buyer ID column
    top_buyers_plot <- tryCatch(
      plot_top_buyers_single_bid(df, buyer_id_col = NULL,
  ...
  if (isTRUE(save_outputs)) {
    ...
    if (!is.null(supplier_entry_agg)) save_plot(supplier_entry_agg, output_dir, "supplier_entry_agg.png", width = 10, height = 6)
  ...
  results <- list(
    ...
    supplier_entry_agg = supplier_entry_agg,   # used when CPV data is absent
