# ============================================================================
# PROCUREMENT INTEGRITY UTILITIES — integrity_utils.R
# ============================================================================
# Config, data-quality, missing-value diagnostics (MCAR/MAR), buyer/supplier
# concentration, single-bid & relative-price regressions, market anomaly
# detection, and the batch pipeline runner.
# Sourced THIRD by global.R. Helpers shared with the admin module are defined
# in utils_shared.R — cross-reference pointers below say exactly where.
# analyze_buyer_supplier_concentration is REPLACED at runtime by app-level
# code ([APP-G03] in global.R) — a deliberate override.
#
...
# TABLE OF CONTENTS
# -----------------
#   [IN-01]  PART 1: CONFIGURATION (create_pipeline_config); year windows: utils_shared [SH-12]
#   [IN-02]  PART 2: LABEL DEFINITIONS AND LOOKUPS (label_lookup, label_with_lookup)
#   [IN-03]  PART 3: STANDARD PLOT THEME (standard_plot_theme, white_bg)
#   [IN-04]  PART 4: DATA VALIDATION (validate_required_columns, check_data_quality); load_data: utils_shared [SH-06]
#   [IN-05]  PART 5: DATA PREPARATION (standardize_missing_values, add_derived_variables, prepare_data); add_buyer_group: utils_shared [SH-09]
#   [IN-06]  PART 6: MISSING-VALUE SHARE COMPUTATIONS
#   [IN-07]  PART 7: MISSINGNESS PLOTS (bars, heatmaps, plot saver)
#   [IN-08]  PART 8: CO-OCCURRENCE, LITTLE'S MCAR TEST, MAR PREDICTABILITY
#   [IN-09]  PART 9: MODEL SPEC HELPERS (vcov robustness trio; core helpers: utils_shared [SH-13])
#   [IN-10]  PART 10: MODEL SELECTION (pick_most_robust_model, model_diagnostics; pick_best_model: utils_shared [SH-14])
#   [IN-11]  PART 11: DISPLAY HELPERS (pretty labels, FE notes, heatmap builders)
#   [IN-12]  PART 12: MODULE — MISSING VALUE ANALYSIS (analyze_missing_values, run_missing_advanced_tests)
#   [IN-13]  PART 13: MODULE — INTEROPERABILITY (analyze_interoperability)
#   [IN-14]  PART 14: MODULE — COMPETITION (concentration — analyze_buyer_supplier_concentration is OVERRIDDEN at runtime by global.R [APP-G03]; single-bid regressions)
#   [IN-15]  PART 15: MODULE — MARKETS (unusual entries, flow matrix, network graph)
#   [IN-16]  PART 16: MODULE — PRICES (relative-price prep + regressions)
#   [IN-17]  PART 17: MODULE — REGIONAL (analyze_regional)
#   [IN-18]  PART 18: SUMMARY STATISTICS (log_summary_stats)
#   [IN-19]  PART 19: SAFE MODULE EXECUTION (safely_run_module)
#   [IN-20]  PART 20: OUTPUT DIRECTORY (ensure_output_directory)
#   [IN-21]  PART 21: MAIN PIPELINE (run_integrity_pipeline — full/batch version; the app uses run_integrity_pipeline_fast_local in app.R [APP-G24])
#
# Anchor codes IN-xx mirror the existing 'PART n' numbering of this file.
# ============================================================================
...
# [SH-12]. Source utils_shared.R before this file.
...
# [IN-03] PART 3: STANDARD PLOT THEME (standard_plot_theme, white_bg) ──────
# ========================================================================
# PART 3: STANDARD PLOT THEME SETTINGS
# ========================================================================
...
# load_data() is defined in utils_shared.R [SH-06] (it drops duplicated
# columns and coerces date columns to character). The interactive app's
# upload path uses its own reader in server.R [APP-SV03].
...
# [IN-05] PART 5: DATA PREPARATION (standardize_missing_values, add_derived_variables, prepare_data; add_buyer_group: utils_shared [SH-09]) ────
# ========================================================================
# PART 5: DATA PREPARATION HELPERS
# ========================================================================

# add_buyer_group() is defined in utils_shared.R [SH-09].
...
add_derived_variables <- function(df) {
  df <- df %>%
    dplyr::mutate(
      has_buyer_id  = if ("buyer_masterid"  %in% names(df)) !is.na(buyer_masterid)  else NA,
      has_bidder_id = if ("bidder_masterid" %in% names(df)) !is.na(bidder_masterid) else NA,
      has_price     = (if ("bid_price"    %in% names(df)) !is.na(bid_price)    else FALSE) |
        (if ("bid_priceusd" %in% names(df)) !is.na(bid_priceusd) else FALSE)
    )
...
#' @param height_per_row   Ignored; retained for call-compatibility
#' @param text_size        Ignored; retained for call-compatibility
#' @return ggplot object
...
# [IN-08] PART 8: CO-OCCURRENCE, LITTLE'S MCAR TEST, MAR PREDICTABILITY ────
# ========================================================================
# PART 8: MISSINGNESS CO-OCCURRENCE, MCAR TEST, MAR PREDICTABILITY
# ========================================================================
...
run_little_mcar_test <- function(df, max_cols = 20) {
  ...
  df_num  <- df_core %>%
    ...
    # Usable columns only: some observed AND some missing values, non-zero
    # variance. Text/date/ID columns coerce to all-NA above and would
    # otherwise crowd out the real numeric fields under the max_cols cap,
    # making every pattern covariance non-invertible (p-value always NA).
    dplyr::select(dplyr::where(~ {
      ms <- mean(is.na(.))
      ms > 0 && ms < 1 && isTRUE(stats::var(., na.rm = TRUE) > 0)
    }))
  if (ncol(df_num) > max_cols) {
    ord    <- order(abs(colMeans(is.na(df_num)) - 0.5))  # most informative first
    df_num <- df_num[, ord[seq_len(max_cols)], drop = FALSE]
  }
  # Standardise: Little's statistic is scale-invariant, and standardising
  # keeps the per-pattern covariance sub-matrices well-conditioned.
  if (ncol(df_num) >= 2) df_num <- as.data.frame(scale(df_num))
  ...
  tryCatch({
    ...
    # Encode patterns as 0/1 strings. Pasting logicals gives "TRUEFALSE..."
    # whose per-character split yields letters like "R"/"U" that coerce to NA
    # — crashing the sum(obs) test below on every dataset.
    pat_str  <- apply(is.na(df_num), 1,
                      function(r) paste(as.integer(r), collapse = ""))
    pat_tab  <- table(pat_str)
    
    d2 <- 0; df_acc <- 0
    pat_tab <- pat_tab[pat_tab >= 5]   # tiny patterns only add noise
    for (pat in names(pat_tab)) {
      obs  <- strsplit(pat, "")[[1]] == "0"   # "0" = observed
      if (sum(obs) == 0) next
      ...
      if (any(is.na(sig_k)) || any(is.na(diff))) next
      tryCatch({
        # small ridge keeps near-singular pattern covariances invertible
        sig_r  <- sig_k + diag(1e-8 * max(diag(sig_k), 1), nrow(sig_k))
        d2     <- d2 + n_k * as.numeric(t(diff) %*% solve(sig_r) %*% diff)
    ...
    if (df_acc <= p) {
      return(list(statistic = NA_real_, df = NA_integer_, p_value = NA_real_,
                  n_cols = p, n_rows = n,
                  interpretation = paste0(
                    "The test could not accumulate enough usable missingness ",
                    "patterns (each needs \u22655 rows and an invertible ",
                    "covariance sub-matrix). The MAR regressions below are the ",
                    "more informative diagnostic for this dataset.")))
    }
...
run_mar_predictability <- function(df, label_lookup, min_miss = 0.01) {
  ...
  purrr::map_dfr(target_vars, function(var) {
    ...
    ybar <- mean(fit_df$y)
    if (ybar < 0.002 || ybar > 0.998) return(NULL)  # (near-)constant outcome
    # Lump rare factor levels: dozens of tiny categories cause perfect
    # separation (the "fitted probabilities 0 or 1" / non-convergence
    # warnings) without adding explanatory information.
    for (v in intersect(c("buyer_type", "proc_type"), names(fit_df)))
      fit_df[[v]] <- forcats::fct_lump_n(factor(fit_df[[v]]), n = 8,
                                         other_level = "Other (rare)")
    fml    <- stats::as.formula(paste("y ~", paste(pred_avail, collapse = " + ")))
    m_full <- tryCatch(suppressWarnings(
      stats::glm(fml, data = fit_df, family = stats::binomial(),
                 control = stats::glm.control(maxit = 50))),
      error = function(e) NULL)
    m_null <- tryCatch(suppressWarnings(
      stats::glm(y ~ 1, data = fit_df, family = stats::binomial())),
      error = function(e) NULL)
...
plot_ggeffects_line <- function(pred,
  ...
  # Build a plain data.frame from the ggeffects object.
  # Column NAMES vary by ggeffects version so we use multi-strategy detection.
  ...
  pred_df <- data.frame(
    x         = as.numeric(x_col),
    predicted = as.numeric(y_col),
    conf.low  = as.numeric(lo_col),
    conf.high = as.numeric(hi_col),
    stringsAsFactors = FALSE
  )
  
  # If CIs are all NA, try to compute from std.error column
  if (all(is.na(pred_df$conf.low))) {
    se_col <- if ("std.error" %in% cn) df[["std.error"]]
    else { i <- grep("std.*err|^se$", cn, ignore.case = TRUE); if (length(i)) df[[i[1]]] else NULL }
    if (!is.null(se_col)) {
      pred_df$conf.low  <- pred_df$predicted - 1.96 * as.numeric(se_col)
      pred_df$conf.high <- pred_df$predicted + 1.96 * as.numeric(se_col)
    }
  }
  
  # If still NA, create a narrow band (±5% of predicted) so ribbon is visible
  if (all(is.na(pred_df$conf.low))) {
    pred_df$conf.low  <- pred_df$predicted * 0.95
    pred_df$conf.high <- pred_df$predicted * 1.05
  }
  ...
  # Use same styling as admin efficiency regression plots (base_size = 20)
  p <- ggplot2::ggplot(pred_df, ggplot2::aes(x = x, y = predicted)) +
...
# [IN-09] PART 9: MODEL SPEC HELPERS (vcov robustness trio; core helpers: utils_shared [SH-13]) ────
# ========================================================================
# PART 9: MODEL SPECIFICATION HELPERS
# ========================================================================

# make_fe_part / make_cluster / safe_fixest / extract_effect_fixest are
# defined in utils_shared.R [SH-13]. The vcov-robustness helpers below
...
# effect_p10_p90() is defined in utils_shared.R [SH-13].
...
# [IN-10] PART 10: MODEL SELECTION (pick_most_robust_model, model_diagnostics; pick_best_model: utils_shared [SH-14]) ────
# ========================================================================
# PART 10: MODEL SELECTION FUNCTIONS
# ========================================================================

# pick_best_model() [SH-14] and the sensitivity/robustness suite [SH-15]
# are defined in utils_shared.R. pick_most_robust_model() and
# model_diagnostics() below are specific to this module.
...
# [IN-12] PART 12: MODULE — MISSING VALUE ANALYSIS (analyze_missing_values, run_missing_advanced_tests) ────
# ========================================================================
# PART 12: MODULE - MISSING VALUE ANALYSIS (with dynamic heights)
# ========================================================================
...
# [IN-13] PART 13: MODULE — INTEROPERABILITY (analyze_interoperability) ──────
# ========================================================================
# PART 13: MODULE - INTEROPERABILITY ANALYSIS
# ========================================================================
...
# [IN-14] PART 14: MODULE — COMPETITION (concentration — analyze_buyer_supplier_concentration is OVERRIDDEN at runtime by global.R [APP-G03]; single-bid regressions) ────
# ========================================================================
# PART 14: MODULE - COMPETITION ANALYSIS
# ========================================================================
...
build_concentration_yearly_plot <- function(yearly_data, n_buyers = 15,
  ...
  real_df <- yearly_data %>%
    dplyr::filter(total_contracts >= min_contracts, is.finite(max_conc)) %>%
    dplyr::group_by(tender_year) %>%
    dplyr::slice_max(max_conc, n = n_buyers, with_ties = FALSE) %>%
    dplyr::ungroup() %>%
    dplyr::mutate(
      # Three-tier priority: buyer_masterid > buyer_id > buyer_name.
      # The buyer_masterid column holds the best available identifier — .absc_impl
      # resolved the tier (masterid → id → name) before renaming to buyer_masterid.
      # So using buyer_masterid directly honours the requested priority without any
      # conditional, and avoids showing a human-readable name when an ID is available.
      disp_name   = as.character(buyer_masterid),
      label_short = dplyr::if_else(nchar(disp_name) > 16,
                                   paste0(substr(disp_name, 1, 15), "\u2026"),
                                   disp_name),
      tooltip = paste0(
        "<b>", disp_name, "</b><br>",
        "Year: <b>", tender_year, "</b><br>",
        "Max concentration: <b>", scales::percent(max_conc, accuracy = 1), "</b><br>",
        "Contracts: ", scales::comma(total_contracts), "<br>",
        dplyr::if_else(repeated,
                       paste0("<b>Also in: ", years_list, "</b><br><i>Persistent risk</i>"),
                       "<i>Single-year only</i>")
      ),
      bar_col = dplyr::if_else(repeated, "#e74c3c", "#5dade2")
    )
  
  if (nrow(real_df) == 0) return(NULL)
  
  years  <- sort(unique(real_df$tender_year))
  n_yrs  <- length(years)
  # Two columns: three-wide panels are too narrow for buyer labels once the
  # figure sits in a dashboard column. The server-side container height
  # formula in output$integ_concentration_plot_ui must stay in sync.
  n_cols <- min(n_yrs, 2L)
  n_rows <- ceiling(n_yrs / n_cols)
  
  # Build one native plotly horizontal bar chart per year.
  # Every panel is given EXACTLY n_buyers slots so all subplots have identical
  # dimensions — no ggplotly conversion artefacts (unequal widths, text size drift).
  make_yr_trace <- function(yr, show_xaxis_title) {
    d <- real_df %>%
      dplyr::filter(tender_year == yr) %>%
      dplyr::arrange(max_conc)   # ascending so highest is at top
    
    n_real <- nrow(d)
    n_pad  <- n_buyers - n_real
    
    # Unique but human-readable pad labels — plotly sees unique categories
    # so allocates equal bar height per row.  We suppress their display via
    # tickvals/ticktext below so the user only sees real buyer names.
    pad_labels <- if (n_pad > 0)
      paste0(".pad.", seq_len(n_pad), ".")
    else
      character(0)
    
    d_sel <- d %>%
      dplyr::mutate(disp_label = label_short,
                    # white on the red (repeated-risk) bars, navy on light blue
                    lbl_col    = dplyr::if_else(repeated, "#ffffff", "#14324a")) %>%
      dplyr::select(label_short, disp_label, lbl_col, max_conc, bar_col,
                    tooltip, total_contracts)
    
    if (n_pad > 0) {
      pad_df <- tibble::tibble(
        label_short     = pad_labels,
        disp_label      = "",
        lbl_col         = "rgba(0,0,0,0)",
        max_conc        = 0,
        bar_col         = "rgba(0,0,0,0)",
        tooltip         = "",
        total_contracts = 0L
      )
      # Padding goes FIRST (bottom of chart); real buyers on top
      d_full <- dplyr::bind_rows(pad_df, d_sel)
    } else {
      d_full <- d_sel
    }
    ...
    plotly::plot_ly(
      ...
      type         = "bar",
      orientation  = "h",
      marker       = list(color = ~bar_col, line = list(color = "white", width = 0.5)),
      hovertext    = ~tooltip,
      hoverinfo    = "text",
      # Buyer names are drawn ON the bars (inside long bars, to the right of
      # short ones). Per-panel y tick labels + automargin inside subplot()
      # give each panel a different plot-area width and can overflow into
      # the neighbouring panel — the cause of uneven facets and labels
      # escaping their borders.
      text             = ~disp_label,
      # "inside" + constraintext "none": labels anchor at the bar base and
      # may overflow a short bar into the panel, but can never be shrunk to
      # invisibility (default constraintext) nor placed outside the panel
      # borders (textposition "auto"/"outside" on near-full bars).
      textposition     = "inside",
      insidetextanchor = "start",
      constraintext    = "none",
      textfont         = list(size = 11, color = ~lbl_col),
      showlegend   = FALSE
    ) %>%
      plotly::layout(
        xaxis = list(
          tickformat = ".0%",
          tickfont   = list(size = 11, color = "#444444"),
          title      = if (show_xaxis_title)
            list(text = "Max share to one supplier", font = list(size = 11.5)) else "",
          range      = c(0, 1.05),
          fixedrange = TRUE,
          gridcolor  = "#eeeeee",
          zeroline   = FALSE
        ...
        yaxis = list(
          title          = "",
          categoryarray  = d_full$label_short,  # fixed order (pads at bottom)
          categoryorder  = "array",
          showticklabels = FALSE,   # names are on the bars — see trace note
          fixedrange     = TRUE
        ),
        bargap = 0.28,
        annotations = list(list(
          ...
          bgcolor   = "#f0f4f8", borderpad = 3
        )),
        margin        = list(t = 28, b = if (show_xaxis_title) 42 else 18, l = 6, r = 6),
        plot_bgcolor  = "#ffffff",
        paper_bgcolor = "#ffffff"
      )
  }
  
  # Determine which panels sit on the bottom row (get the x-axis title)
  bottom_row_start <- (n_rows - 1L) * n_cols + 1L
  panels <- lapply(seq_along(years), function(i) {
    make_yr_trace(years[[i]], show_xaxis_title = (i >= bottom_row_start))
  })
  
  # Complete the grid: plotly::subplot leaves a partially filled last row
  # otherwise (e.g. 10 years in a 3-wide grid \u2192 3+3+3+1), which reads as
  # unequal facet sizes. Invisible blank panels keep every year identical.
  n_cells <- n_rows * n_cols
  if (length(panels) < n_cells) {
    blank <- plotly::plotly_empty(type = "scatter", mode = "markers") %>%
      plotly::layout(xaxis = list(visible = FALSE),
                     yaxis = list(visible = FALSE))
    panels <- c(panels, rep(list(blank), n_cells - length(panels)))
  }
  
  # Equal column widths and row heights via explicit vectors
  col_widths <- rep(1 / n_cols, n_cols)
  row_heights <- rep(1 / n_rows, n_rows)
  ...
  fig <- plotly::subplot(
    ...
    plotly::layout(
      ...
      hovermode     = "closest",
      # Explicit height: n_buyers bar slots per panel need real pixels; an
      # auto-sized figure squeezed into a short container squashes the bars
      # and their labels. Mirrors the container formula in the server.
      height        = n_rows * (n_buyers * 21 + 104) + 50,
      # Global base font: all tick labels inherit this unless explicitly overridden.
      # This is the only reliable way to ensure uniform font size across all subplots,
      # because per-subplot tickfont settings can be lost during axis renaming.
      font          = list(size = 11.5, color = "#333333"),
      # t = 44: the per-panel year badges render ABOVE the panels (yanchor
      # "bottom" at y = 1); with t = 10 the top row's badges were clipped
      # outside the container.
      margin        = list(l = 10, r = 10, t = 44, b = 46),
      plot_bgcolor  = "#ffffff",
      paper_bgcolor = "#ffffff",
      annotations   = list(
        list(
          text      = "<span style='color:#e74c3c;'>&#9632; Appears in multiple years</span>",
          x = 0.25, xref = "paper", y = -0.02, yref = "paper",
          xanchor = "center", yanchor = "top", showarrow = FALSE,
          font = list(size = 11)
        ),
        list(
          text      = "<span style='color:#5dade2;'>&#9632; Single year only</span>",
          x = 0.75, xref = "paper", y = -0.02, yref = "paper",
          xanchor = "center", yanchor = "top", showarrow = FALSE,
          font = list(size = 11)
        )
      )
  ...
  # Force all y-axes and x-axes to identical tickfont — subplot() renames axes
  # (yaxis, yaxis2, yaxis3 ...) and per-panel settings may not merge correctly.
  pb <- plotly::plotly_build(fig)
  for (ax in names(pb$x$layout)) {
    if (grepl("^yaxis", ax)) {
      pb$x$layout[[ax]]$tickfont   <- list(size = 10, color = "#333333")
      pb$x$layout[[ax]]$automargin <- TRUE
    }
    if (grepl("^xaxis", ax)) {
      pb$x$layout[[ax]]$tickfont   <- list(size = 10, color = "#444444")
    }
  }
  pb
}
...
analyze_buyer_supplier_concentration <- function(df, config) {
  ...
  # Three-tier paired ID resolution: both buyer and bidder columns must come
  # from the same tier so concentration is computed on a consistent entity.
  #   Tier 1: buyer_masterid + bidder_masterid  (harmonised IDs)
  #   Tier 2: buyer_id       + bidder_id        (raw admin IDs)
  #   Tier 3: buyer_name     + bidder_name      (name strings)
  .id_pairs <- list(
  ...
  for (.p in .id_pairs) {
    if (all(.p %in% names(df)) &&
        any(!is.na(df[[.p[1]]]) & nchar(as.character(df[[.p[1]]])) > 0) &&
        any(!is.na(df[[.p[2]]]) & nchar(as.character(df[[.p[2]]])) > 0)) {
      .chosen <- .p; break
    }
  }
  if (is.null(.chosen)) {
    message("Skipping buyer-supplier concentration: no valid buyer/bidder ID pair found ",
            "(checked buyer_masterid/bidder_masterid, buyer_id/bidder_id, buyer_name/bidder_name)")
    return(results)
  }
  buyer_col  <- .chosen[1]
  bidder_col <- .chosen[2]
  message("Buyer-supplier concentration: using '", buyer_col, "' + '", bidder_col, "'")
  
  # Best available price column; fall back to contract count weight (1) if none found
  ...
  # Normalise to generic column aliases so the rest of the pipeline is column-agnostic
  d <- df %>%
    dplyr::mutate(
    ...
    dplyr::filter(!is.na(.buyer),  nchar(.buyer)  > 0,
                  !is.na(.bidder), nchar(.bidder) > 0,
                  !is.na(tender_year))
  
  if (nrow(d) == 0) return(results)
  ...
  results$data <- d %>%
    ...
    dplyr::mutate(
      ...
      buyer_concentration  = dplyr::if_else(
    ...
    dplyr::mutate(
      n_suppliers = dplyr::n(),
      buyer_concentration_display = dplyr::if_else(
        n_suppliers < (config$thresholds$min_suppliers_for_buyer_conc %||% 1L),
        NA_real_,
        buyer_concentration
      )
    ) %>%
    dplyr::ungroup() %>%
    # Rename generic aliases to the canonical names expected by downstream helpers
    dplyr::rename(buyer_masterid = .buyer, bidder_masterid = .bidder) %>%
    dplyr::filter(!is.na(buyer_concentration_display))
  
  if (nrow(results$data) == 0) return(results)
  
  # Buyer display-name lookup (best effort)
  nm_lkp <- d %>%
  ...
  top_buyers <- results$data %>%
    ...
    dplyr::slice_max(max_conc, n = config$thresholds$top_n_buyers %||% 20L)
  ...
  top_buyers_yearly_raw <- results$data %>%
    ...
    dplyr::filter(is.finite(max_conc)) %>%
    dplyr::left_join(nm_lkp, by = "buyer_masterid")
  ...
  results$yearly_data <- top_buyers_yearly_raw %>%
    ...
    dplyr::mutate(
      ...
      # Three-tier buyer ID priority: buyer_masterid column already holds the best
      # available identifier (masterid → id → name resolution happened above in
      # buyer_col selection). Use it directly so plots show the ID, not the name.
...
run_singleb_specs <- function(buyer_analysis_fe, config) {
  out <- list()
  k <- 0L
  n_total <- nrow(buyer_analysis_fe)
    ...
    for (fe in config$models$fe_set) {
      ...
      for (cl in config$models$cluster_set) {
        ...
        for (ctrl in config$models$controls_set) {
          ...
          rhs_terms <- rhs_terms[
            grepl("\\(", rhs_terms) |
              rhs_terms %in% names(buyer_analysis_fe)
          ]
          if (length(rhs_terms) == 0 || !("cumulative_missing_share" %in% rhs_terms)) next
          rhs <- paste(rhs_terms, collapse = " + ")
          fml <- stats::as.formula(paste0("cumulative_singleb_rate ~ ", rhs, " | ", fe_part))
          
          m <- switch(mt,
                      "fractional_logit" = safe_fixest(
                        fixest::feglm(fml, family = quasibinomial(link = "logit"),
                                      data = buyer_analysis_fe, cluster = cl_fml)),
                      "lpm" = safe_fixest(
                        fixest::feols(fml, data = buyer_analysis_fe, cluster = cl_fml)),
                      "probit" = safe_fixest(
                        fixest::feglm(fml, family = binomial(link = "probit"),
                                      data = buyer_analysis_fe, cluster = cl_fml)),
                      NULL
          )
          ...
          eff <- tryCatch(
            extract_effect_fixest(model = m, x_name = "cumulative_missing_share", data_used = buyer_analysis_fe),
...
analyze_singleb_data <- function(df, config) {
  ...
  # Normalise column structure so we get buyer_masterid, buyer_buyertype etc.
  df <- normalize_procurement_data(df)
  
  # Detect which buyer column to use for grouping
  .buyer_col <- if ("buyer_masterid" %in% names(df)) "buyer_masterid"
  else if ("buyer_id" %in% names(df)) "buyer_id"
  else NULL
  if (is.null(.buyer_col)) {
    message("Skipping single-bidding analysis: no buyer identifier column found.")
    return(results)
  }
  if (.buyer_col != "buyer_masterid")
    df$buyer_masterid <- df[[.buyer_col]]
  
  message("  Singleb: using buyer column '", .buyer_col, "' with ",
          dplyr::n_distinct(df$buyer_masterid), " unique buyers")
  
  # Scale to 0-1
  .singleb_max <- max(df$ind_corr_singleb, na.rm = TRUE)
  message("  Singleb: ind_corr_singleb max = ", .singleb_max,
          " (", if (.singleb_max > 1) "0-100 scale, dividing by 100" else "already 0-1", ")")
  df <- df %>%
    dplyr::mutate(
      ind_corr_singleb = dplyr::if_else(
        !is.na(ind_corr_singleb),
        if (.singleb_max > 1) ind_corr_singleb / 100 else as.numeric(ind_corr_singleb),
        NA_real_
      )
    )
  ...
  message("  Singleb: year filter [", yr$min_year, ", ", yr$max_year, "]")
  df_filtered <- df %>%
    dplyr::filter(
      !is.na(tender_year),
  ...
  message("  Singleb: ", nrow(df_filtered), " rows after year filter")
  ...
  buyer_controls <- {
    # Detect best available price column for contract value controls
    .pc <- NA_character_
    for (.c in c("bid_priceusd", "bid_price", "lot_estimatedpriceusd", "lot_estimatedprice",
                 "contractsign_amount_ghs", "contractsign_amount"))
      if (.c %in% names(df_filtered) && any(!is.na(df_filtered[[.c]]))) { .pc <- .c; break }
    message("  Singleb: price column for controls = ", if (is.na(.pc)) "NONE" else .pc)
    
    # Group by buyer + year (+ buyer_buyertype if present)
    grp_cols <- c("buyer_masterid", "tender_year")
    if ("buyer_buyertype" %in% names(df_filtered)) grp_cols <- c(grp_cols, "buyer_buyertype")
    
    bc <- df_filtered %>% dplyr::group_by(dplyr::across(dplyr::all_of(grp_cols)))
    if (!is.na(.pc))
      bc <- dplyr::summarise(bc,
                             n_contracts          = dplyr::n(),
                             avg_contract_value   = mean(.data[[.pc]], na.rm = TRUE),
                             total_contract_value = sum(.data[[.pc]],  na.rm = TRUE),
                             .groups = "drop"
      )
    else
      bc <- dplyr::summarise(bc,
                             n_contracts          = dplyr::n(),
                             avg_contract_value   = NA_real_,
                             total_contract_value = NA_real_,
                             .groups = "drop"
      )
    bc
  }
  
  # Add buyer_buyertype column if it's missing from buyer_controls (can happen
  # when buyer_buyertype didn't exist in df_filtered and wasn't in the grouping)
  if (!"buyer_buyertype" %in% names(buyer_controls))
    buyer_controls$buyer_buyertype <- NA_character_
  
  # Ensure one row per buyer-year (buyer_buyertype grouping may have created dupes)
  buyer_controls <- buyer_controls %>%
    dplyr::group_by(buyer_masterid, tender_year) %>%
    dplyr::summarise(
      n_contracts          = sum(n_contracts, na.rm = TRUE),
      avg_contract_value   = mean(avg_contract_value, na.rm = TRUE),
      total_contract_value = sum(total_contract_value, na.rm = TRUE),
      buyer_buyertype      = dplyr::first(buyer_buyertype),
      .groups = "drop"
    )
  
  message("  Singleb: buyer_missing_share = ", nrow(buyer_missing_share), " rows, ",
          "buyer_singleb_rate = ", nrow(buyer_singleb_rate), " rows, ",
          "buyer_controls = ", nrow(buyer_controls), " rows")
  ...
  buyer_analysis <- buyer_missing_share %>%
    ...
    dplyr::mutate(
      buyer_buyertype = {
        .f <- as.factor(buyer_buyertype)
        if (any(is.na(.f))) {
          levels(.f) <- c(levels(.f), "Unknown")
          .f[is.na(.f)] <- "Unknown"
        }
        .f
      }
  ...
  message("  Singleb: buyer_analysis after joins = ", nrow(buyer_analysis), " rows")
  ...
  message("  Singleb: ", length(eligible_buyers), " buyers with >= ",
          config$thresholds$min_buyer_years, " years of data")
  ...
  message("  Singleb: final panel = ", nrow(buyer_analysis_fe), " buyer-year observations")
...
analyze_competition <- function(df, config, output_dir, run_regressions = FALSE, save_plots = TRUE) {
  ...
  # Wrapped in tryCatch so that a crash here does not prevent singleb regressions
  tryCatch({
  ...
  }, error = function(e) message("Buyer-supplier concentration skipped: ", e$message))
  ...
  if (isTRUE(run_regressions)) {
    ...
    singleb <- tryCatch(
      analyze_singleb_data(df, config),
      error = function(e) {
        message("  analyze_singleb_data ERROR: ", e$message)
        list()
      })
    if (length(singleb) > 0 && !is.null(singleb$data)) {
      results$singleb_data <- singleb$data
      message("  Singleb data ready: ", nrow(results$singleb_data), " rows")
    } else {
      message("  Singleb data prep returned empty — skipping regressions.")
    }
    ...
    if (!is.null(results$singleb_data) && nrow(results$singleb_data) > 0) {
      ...
      if (!is.null(specs) && nrow(specs) > 0) {
        ...
        # Determine robustness from bundle (same criteria as UI verdict)
        .is_robust <- {
          bnd <- results$singleb_sensitivity
          has_rows <- function(tbl) !is.null(tbl) && is.data.frame(tbl) && nrow(tbl) > 0
          sp <- if (has_rows(bnd$overall)) bnd$overall$share_positive else 0
          cn <- if (has_rows(bnd$overall)) names(bnd$overall) else character(0)
          p10_col <- cn[grepl("share_p_le_0.1", cn, fixed = TRUE)]
          p10 <- if (length(p10_col) > 0) bnd$overall[[p10_col[1]]] else 0
          ss <- if (has_rows(bnd$sign)) bnd$sign$share_sign_stable else 0
          isTRUE(sp >= 0.7 && p10 >= 0.6 && ss == 1)
        }
        # Fallback: no significant spec — use scoring without p-value filter
        if (is.null(best_row)) {
          best_row <- pick_best_model(specs, require_positive = FALSE, p_max = 1.0,
                                      strength_col = "effect_strength",
                                      preferred_model = "fractional_logit")
          message("  Single-bidding: no significant spec found — showing best-scoring available model.")
        }
        if (!is.null(best_row) && nrow(best_row) > 0) {
          ...
          fml   <- stats::as.formula(paste0("cumulative_singleb_rate ~ ",
                                            ...
                                            " | ", fe_part))
          model <- tryCatch(switch(best_row$model_type,
                                   "lpm"    = fixest::feols(fml, data = results$singleb_data, cluster = cl_fml),
                                   "probit" = fixest::feglm(fml, family = binomial(link = "probit"),
                                                            data = results$singleb_data, cluster = cl_fml),
                                   fixest::feglm(fml, family = quasibinomial(link = "logit"),
                                                 data = results$singleb_data, cluster = cl_fml)
          ), error = function(e) {
            message("  Singleb best model refit failed: ", e$message)
            NULL
          })
          pred <- if (!is.null(model))
            tryCatch(ggeffects::ggpredict(model, terms = "cumulative_missing_share"),
                     error = function(e) { message("  Singleb ggpredict failed: ", e$message); NULL })
          else NULL
          if (!is.null(pred)) {
            # Use actual years from the data, not config (which may be -Inf/Inf)
            actual_years <- range(results$singleb_data$tender_year, na.rm = TRUE)
            if (any(is.infinite(actual_years)))
              actual_years <- range(config$years_singleb$min_year, config$years_singleb$max_year)
            yr_min <- if (is.infinite(actual_years[1])) "?" else actual_years[1]
            yr_max <- if (is.infinite(actual_years[2])) "?" else actual_years[2]
            robustness_tag <- if (.is_robust) "\u2713 Robust" else "\u2717 NOT ROBUST \u2014 best available model"
            results$singleb_plot <- plot_ggeffects_line(
              pred     = pred,
              title    = paste("Predicted Single-Bidding by Missing Share \u2013", config$country),
              subtitle = paste0(
                robustness_tag, " | Model: ", pretty_model_name(best_row$model_type),
                " | Years: ", yr_min, "\u2013", yr_max,
                " | N=", best_row$nobs,
              ...
              caption = paste0(
                "Filters: tender_year in [", yr_min, ", ", yr_max,
                "]; buyers with \u2265", config$thresholds$min_buyer_years, " years of data. ",
                controls_note(best_row$controls),
                if (.is_robust) " Selected: highest quality score among significant specs, closest to median estimate."
                else " Selected: highest quality score (no spec met robustness criteria). Interpret with caution."
              )
            )
            save_plot(results$singleb_plot, "reg_singleb_missing_share")
          }
          results$singleb_best_row  <- best_row
          results$singleb_is_robust <- .is_robust
        }
      }
...
# [IN-15] PART 15: MODULE — MARKETS (unusual entries, flow matrix, network graph) ────
# ========================================================================
# PART 15: MODULE - MARKET ANALYSIS
# ========================================================================
...
build_network_graph_from_matrix <- function(unusual_matrix, min_bidders = 4,
  ...
  ggraph::ggraph(g, layout = "fr") +
    ...
    # Expanded layout limits + clip off: repelled node labels near the hull
    # of the layout must never be cut at the panel border.
...
analyze_markets <- function(df, config, output_dir) {
  ...
  # Batch plot rendering is non-fatal: results$unusual_matrix and
  # results$unusual_entries are already stored above, and the interactive
  # tabs rebuild their own plots from the matrix. A failure here must not
  ...
  if (nrow(edges_markets) > 0) {
    ...
    message("analyze_markets: batch flow/network plot rendering failed (",
            "interactive tabs are unaffected): ", e$message))
  
  # Supplier-level unusual behavior — non-fatal (see note above)
  tryCatch({
  ...
  save_plot(results$supplier_unusual_plot, "supp_unusual_suppliers")
    ...
    message("analyze_markets: supplier-level plot block failed: ", e$message))
  
  # Market-level unusual entries — non-fatal (see note above)
  tryCatch({
  ...
  results$market_unusual_plot <- ggplot2::ggplot(
    ...
    # Headroom on both axes: ggplotly renders the labels at their raw point
    # positions (repel is print-only), so without expansion they can land
    # outside the panel borders.
    ggplot2::scale_x_continuous(expand = ggplot2::expansion(mult = c(0.12, 0.18))) +
    ggplot2::scale_y_continuous(expand = ggplot2::expansion(mult = c(0.10, 0.16))) +
  ...
  save_plot(results$market_unusual_plot, "supp_unusual_markets")
    ...
    message("analyze_markets: market-level plot block failed: ", e$message))
...
prepare_price_data <- function(df, config) {
  # Price analysis requires both bid_price and lot_estimatedprice.
  # Accept normalised aliases (bid_priceusd) that may have been copied from bid_price.
  .has_bid    <- any(c("bid_price", "bid_priceusd") %in% names(df))
  .has_est    <- "lot_estimatedprice" %in% names(df)
  if (!.has_bid || !.has_est) {
    message("Skipping price analysis: need bid_price + lot_estimatedprice (have bid=", .has_bid, " est=", .has_est, ")")
    return(NULL)
  }
  # Use bid_price directly for relative price calculation (same currency as est.)
  if (!"bid_price" %in% names(df) && "bid_priceusd" %in% names(df))
    df$bid_price <- df$bid_priceusd
  
  # Determine value variable for log contract value control
  ...
  # Compute total missing share — use only columns actually present
  key_vars <- names(label_lookup) %>% gsub("_missing_share", "", .)
  key_vars_present <- intersect(key_vars, names(df))
  if (length(key_vars_present) > 0) {
    df <- df %>%
      dplyr::mutate(
        total_missing_share = rowMeans(
          dplyr::across(dplyr::any_of(key_vars_present), ~ is.na(.)),
          na.rm = TRUE
        )
      )
  } else {
    df$total_missing_share <- NA_real_
  }
  ...
  # Safe relevel: pick the requested reference if it exists, else the most common level
  .safe_relevel <- function(x, preferred) {
    x <- as.factor(x)
    if (preferred %in% levels(x)) return(stats::relevel(x, ref = preferred))
    # Fall back to most common level
    tab <- sort(table(x), decreasing = TRUE)
    if (length(tab) > 0) stats::relevel(x, ref = names(tab)[1]) else x
  }
  
  if ("tender_proceduretype" %in% names(rel_price_data))
    rel_price_data$tender_proceduretype <- .safe_relevel(rel_price_data$tender_proceduretype, "OPEN")
  if ("buyer_buyertype" %in% names(rel_price_data))
    rel_price_data$buyer_buyertype <- .safe_relevel(rel_price_data$buyer_buyertype, "UTILITIES")
    ...
    for (fe in fe_set) {
      ...
      for (cl in config$models$cluster_set) {
        ...
        for (ctrl in config$models$controls_set) {
          ...
          eff <- tryCatch(
            extract_effect_fixest(model = m, x_name = x_name, data_used = d_used),
            error = function(e) list(estimate = NA_real_, pvalue = NA_real_, nobs = NA_integer_, std_slope = NA_real_)
          )
          if (is.na(eff$estimate)) next
          ...
          .n_collinear   <- tryCatch(length(m$collin.var %||% character(0)), error = function(e) 0L)
          .n_eff         <- tryCatch(m$nobs %||% eff$nobs, error = function(e) NA)
          .pct_retained  <- if (!is.na(.n_eff) && nrow(d_used) > 0) .n_eff / nrow(d_used) else NA_real_
          .out_of_range  <- NA_real_  # not applicable for continuous DV
...
analyze_prices <- function(df, config, output_dir) {
  ...
  if (!is.null(results$relprice_sensitivity)) {
    n_sp <- if (!is.null(results$specs)) nrow(results$specs) else 0
    message("  Relative price: ", n_sp, " specifications tested.")
  }
  ...
  best_row <- pick_best_model(
    ...
    preferred_model = "gamma_log"   # Gamma GLM appropriate for positive continuous outcome
  )
  # Determine robustness from bundle (same criteria as UI verdict)
  .is_robust <- {
    bnd <- results$relprice_sensitivity
    has_rows <- function(tbl) !is.null(tbl) && is.data.frame(tbl) && nrow(tbl) > 0
    sp <- if (has_rows(bnd$overall)) bnd$overall$share_positive else 0
    cn <- if (has_rows(bnd$overall)) names(bnd$overall) else character(0)
    p10_col <- cn[grepl("share_p_le_0.1", cn, fixed = TRUE)]
  ...
  # Fallback: no significant spec — use scoring without p-value filter
  if (is.null(best_row)) {
    best_row <- pick_best_model(results$specs, require_positive = FALSE, p_max = 1.0,
                                strength_col = "std_slope",
                                preferred_model = "gamma_log")
    if (is.null(best_row) || nrow(best_row) == 0) best_row <- NULL
    message("  Relative price: no significant spec found — showing best-scoring available model.")
  }
  
  if (is.null(best_row)) {
    message("No relative price model available for ", config$country)
    return(results)
  }
  
  message("Relative price ", if (.is_robust) "BEST" else "BEST AVAILABLE", " spec for ", config$country, ": ",
  ...
  if (best_row$model_type == "ols_level") {
    fml <- stats::as.formula(paste0("relative_price ~ ", rhs, " | ", fe_part))
    model <- tryCatch(fixest::feols(fml, data = results$data, cluster = cl_fml),
                      error = function(e) { message("  Relprice refit failed: ", e$message); NULL })
    if (!is.null(model))
      pred <- tryCatch(ggeffects::ggpredict(model, terms = "total_missing_share"),
    ...
    if (nrow(d) > 0) {
      fml <- stats::as.formula(paste0("log_relative_price ~ ", rhs, " | ", fe_part))
      model <- tryCatch(fixest::feols(fml, data = d, cluster = cl_fml),
                        error = function(e) { message("  Relprice refit failed: ", e$message); NULL })
      if (!is.null(model))
        pred <- tryCatch(ggeffects::ggpredict(model, terms = "total_missing_share"),
                         error = function(e) { message("  ggpredict failed: ", e$message); NULL })
  ...
  if (!is.null(pred)) {
    ...
    yr_min <- if (is.infinite(years_used[1])) "?" else years_used[1]
    yr_max <- if (is.infinite(years_used[2])) "?" else years_used[2]
    robustness_tag <- if (.is_robust) "\u2713 Robust" else "\u2717 NOT ROBUST \u2014 best available model"
  ...
  # Store model selection metadata for UI
  results$relprice_best_row  <- best_row
  results$relprice_is_robust <- .is_robust
...
# [IN-17] PART 17: MODULE — REGIONAL (analyze_regional) ──────────────────────
# ========================================================================
# PART 17: MODULE - REGIONAL ANALYSIS
# ========================================================================
...
# [IN-18] PART 18: SUMMARY STATISTICS (log_summary_stats) ────────────────────
# ========================================================================
# PART 18: SUMMARY STATISTICS MODULE
# ========================================================================
...
# [IN-20] PART 20: OUTPUT DIRECTORY (ensure_output_directory) ────────────────
# ========================================================================
# PART 20: ENSURE OUTPUT DIRECTORY
# ========================================================================
